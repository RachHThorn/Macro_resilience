...
zero_counts <- dat_1 %>%
  ...
  summarise(zero_counts = sum(new_max_cover == 0))
all_counts <- dat_1 %>%
  group_by(trt, year_trt) %>%
  tally() %>%
  rename("all_counts" = n)
...
dat_2 <- dat_2 %>% select(site_name, New_taxon, taxon_site, trt, year_trt, block, new_max_cover) %>%
  group_by(site_name, New_taxon, taxon_site, trt, block) %>%
  pivot_wider(names_from = "year_trt", values_from = "new_max_cover", values_fn = mean) %>% 
  rename("T0" = "0") %>% rename("T2" = "2") %>%
  mutate(T2 = replace_na(T2, 0)) %>%
  rowwise() %>%
...
all <- ggarrange(p1, p2, t1, t2, common.legend = TRUE)
all
ggsave("figures/July_2025/A1_sample_size_zeros.jpeg", all, height = 7.5, width = 12)
