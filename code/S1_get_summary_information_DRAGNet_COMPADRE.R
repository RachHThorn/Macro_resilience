...
# 3) Get list of blocks in DRAGnet surveyed per site and per time point

library(tidyverse) 
library(Rcompadre) # interacts with compadre data-bases
...
Drag_taxa <- read_csv("data/full-cover-2025-07-16.csv") %>%
  dplyr::select(Taxon) %>%
  distinct(Taxon) %>%
